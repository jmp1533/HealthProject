...
public class People_traner_Controller {
	...
	public String join(HttpServletRequest request, HttpServletResponse response) {
		...
		if (js == 0 && (0 <= jc && jc < 5)) {
			...
			sb.append("<msg>Application is completed!</msg>");
		}
		if (js == 1) {
			// 진행중...
			sb.append("<msg>Is waiting for the approval of trainers.</msg>");
		}

		if (jc >= 5) {
			// 5명이상 신청불가...
			sb.append("<msg>Trainers can apply for up to five people.</msg>");
		}
	...
	public String getinfo(@RequestParam("id") String id, Model model) {
		...
		String t_addr = dto.getT_addr().split("#")[0] + " " + dto.getT_addr().split("#")[1];

		dto.setT_addr(t_addr);

                    ...
                    <%--이미지 넣을땐 158*50 픽셀로 만들것! --%>
                        ...
                        <div class="list_1">
                            <%--이미지 링크뒤에 글내용 바꾸고 크기 다시 조절해야함 --%>
                                <a href="/client/menu/finder/company/CompanyList"
                                    style="background-color: red;">헬스장 검색</a>
                ...
                <form method="post" action="/client/menu/finder/company/CompanyList">
                    ...
                    <table border="1">
                        ...
                        <c:forEach var="companydto" items="${requestScope.list}">
                            <tr>
                                <td><a
                                        href="/client/menu/finder/company/Companyview?b_num=${companydto.b_num}">${companydto.b_name}</a>
                                </td>
            ...
            <div class="list_number" style="text-align: center;">
                ...
                <c:choose>
                    <c:when test="${page.startPageNum>10 }">
                        <a
                            href="/client/menu/finder/company/CompanyList?pageNum=${page.startPageNum-1 }">[이전]</a>
                    </c:when>
                    <c:otherwise>
                        [이전]
                    </c:otherwise>
                ...
                <c:forEach var="i" begin="${page.startPageNum}" end="${page.endPageNum}">
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a
                                href="/client/menu/finder/company/CompanyList?pageNum=${i}&b_name=${b_name}">[${i}]</a>
                        </c:otherwise>
                    </c:choose>
                </c:forEach>
                <c:choose>
                    <c:when test="${page.endPageNum<page.totalPageCount}">
                        <a
                            href="/client/menu/finder/company/CompanyList?pageNum=${page.endPageNum+1}&b_name=${b_name}">[다음]</a>
                    </c:when>
                    <c:otherwise>
                        [다음]
                    </c:otherwise>
